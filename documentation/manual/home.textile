...
Module ==<b>@messages</b>== provides a web based tool for managing your application's localizations. You just need to enable the \@messages module in the **conf/application.conf** file
...
In this case the tool will find the first part of the key **user.type.** when the valid key would be **user.type.admin**. To help you with this special, but very common case **@messages** has the **ignore** and **keep* lists.
...
|_. key|_. description|_. default|
| messages.srcDir | A comma separated list of paths the tool uses when it looks for localization keys in the sources. | app |
| messages.targetDir | Path to the directory where the tool will read and write the messages files. | conf |
| messages.resource | Instance of **MessagesResource** class which is used to load and save the localizations | DefaultMessageResource |
